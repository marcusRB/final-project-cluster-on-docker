...
function cleanContainers() {
    ...
    # container="$(docker ps -a | grep 'spark-base-hive' | awk '{print $1}')"
    # docker stop "${container}"
    # docker rm "${container}"
...
function cleanImages() {
    ...
    #   # docker rmi -f "$(docker images | grep -m 1 'spark-worker' | awk '{print $3}')"
    #   # docker rmi -f "$(docker images | grep -m 1 'spark-master' | awk '{print $3}')"
    #   docker rmi -f "$(docker images | grep -m 1 'spark-base-hive' | awk '{print $3}')"
    # fi
...
function buildImages() {
  ...
  then
    ...
    docker build \
      ...
      -t cluster-spark-base:${SPARK_VERSION}-hadoop${HADOOP_VERSION} .
    ...
    docker build \
      ...
      -t cluster-spark-master:${SPARK_VERSION}-hadoop${HADOOP_VERSION} .
    ...
    docker build \
      ...
      -t cluster-spark-worker:${SPARK_VERSION}-hadoop${HADOOP_VERSION} .
