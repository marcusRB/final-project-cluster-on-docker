#!/bin/bash
#
# -- Build Apache Spark - Hive Standalone Cluster Docker Images
...
function cleanContainers() {
    ...
    container="$(docker ps -a | grep 'spark-hive-base' | awk '{print $1}')"
...
function cleanImages() {
    ...
    then
      docker rmi -f "$(docker images | grep -m 1 'spark-hive-base' | awk '{print $3}')"
    fi
...
function buildImages() {
  ...
  then
    ...
    docker build \
      ...
      -f docker/spark-hive-base/Dockerfile \
      -t spark-hive-base:${SPARK_VERSION}-hive${HIVE_VERSION} .
