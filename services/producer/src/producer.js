...
router.post('/sales', (req, res) => {
  const { total, city, country, latitude, longitude } = req.body;
  const parsedTotal = Number(total);
  const parsedCity = String(city);
  const parsedCountry = String(country);
...
router.get('/sales', async (req, res) => {
  const { rows } = await pgClient.query('SELECT id, uuid, total, sale_date, created_at, city, country, latitude, longitude FROM sales');
